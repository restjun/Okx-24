...
def send_new_entry_message(all_ids):
    ...
    for inst_id in all_ids:
        ...
        if not last_status and is_cross:
            ...
            new_entry_coins.append((inst_id, daily_change, volume_24h, h1_mfi, h1_rsi, coin_rank))
    ...
    if new_entry_coins:
        ...
        for inst_id,daily_change,volume_24h,h1_mfi,h1_rsi,coin_rank in new_entry_coins:
            name = inst_id.replace("-USDT-SWAP","")
            volume_str = format_volume_in_eok(volume_24h)
            message_lines.append(
                f"{name}\n거래대금: {volume_str}\n순위: {coin_rank}위\n상승률: {format_change_with_emoji(daily_change)}\n"
                f"📊 1H RSI: {format_rsi_mfi(h1_rsi)} / MFI: {format_rsi_mfi(h1_mfi)}"
            )
