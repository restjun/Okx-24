...
def check_4h_mfi_rsi_cross(inst_id, period=3, threshold=80):  # 70 → 80
...
def send_new_entry_message(all_ids):
    ...
    for inst_id in top_100_ids:
        # 🔹 4시간봉 돌파 체크
        is_cross = check_4h_mfi_rsi_cross(inst_id, period=3, threshold=80)  # 70 → 80
        ...
        if pd.isna(h4_mfi) or h4_mfi<80 or pd.isna(h4_rsi) or h4_rsi<80:  # 70 → 80
            continue
        ...
        if pd.isna(d1_mfi) or d1_mfi<80 or pd.isna(d1_rsi) or d1_rsi<80:  # 70 → 80
            continue
    ...
    if new_entry_coins:
        ...
        message_lines = ["⚡ 4H MFI·RSI 3일선 돌파 + 1D MFI·RSI ≥ 80 필터", "━━━━━━━━━━━━━━━━━━━"]
