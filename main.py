...
# =========================
# 10분봉 RSI/MFI 확인 (둘 다 30 미만)
# =========================

def check_10m_rsi_mfi_below30(inst_id, period=5, threshold=30):
    df = get_ohlcv_okx(inst_id, bar='10m', limit=200)
    if df is None or len(df) < period + 1:
        return False
...
def send_new_entry_message(all_ids):
    ...
    for inst_id in top_ids:
        ...
        # === 추가 조건: 10분봉 RSI/MFI < 30 ===
        if not check_10m_rsi_mfi_below30(inst_id, period=5, threshold=30):
            continue
    ...
    message_lines = ["🆕 당일 신규 돌파 코인 👀 (4H RSI/MFI 30 돌파 + 10m RSI/MFI < 30)"]
