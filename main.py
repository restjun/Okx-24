...
# =========================
# TOP 3 거래대금 기준 (RSI/MFI 70 이상)
# =========================
def get_top3_by_volume(all_ids):
    ...
    for inst_id in top_ids:
        ...
        if (mfi_1d is not None and rsi_1d is not None and mfi_1d >= 70 and rsi_1d >= 70):
            filtered_top.append((inst_id, mfi_1d, rsi_1d, volume_map[inst_id]))
...
def send_new_entry_message(all_ids):
    ...
    for inst_id in top_ids:
        ...
        if not sent_signal_coins[inst_id]["crossed"]:
            new_entry_coins.append(
                (inst_id, daily_change, volume_map.get(inst_id, 0), cross_time)
            )
    ...
    # === TOP 3 진입 코인 확인 ===
    filtered_top = get_top3_by_volume(all_ids)
    new_top3_coins = []
    ...
    # 신규 TOP3 진입
    if new_top3_coins:
        message_lines.append("🏆 TOP 3 거래대금 코인 🌟")
        for rank, (inst_id, mfi_1d, rsi_1d, volume) in enumerate(filtered_top, start=1):
            volume_str = format_volume_in_eok(volume)
            name = inst_id.replace("-USDT-SWAP", "")
            highlight = "🌟" if inst_id in new_top3_coins else ""
            message_lines.append(
                f"{rank}위 {name}{highlight}\n"
                f"💰 {volume_str}M (실거래대금 순위: {rank})\n"
    ...
    if new_entry_coins:
        ...
        message_lines.append("\n🆕 신규 돌파 코인 👀")
        for inst_id, daily_change, volume_24h, cross_time in new_entry_coins:
            name = inst_id.replace("-USDT-SWAP", "")
