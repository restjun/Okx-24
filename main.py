...
# 🔹 전역 변수
sent_signal_coins = {}      # 마지막 4H 돌파 상태 저장
...
def check_4h_mfi_rsi_cross(inst_id, period=3, threshold=70):
    ...
    if df is None or len(df) < period+1: return False
    ...
    if pd.isna(curr_mfi) or pd.isna(curr_rsi): return False
...
def calculate_daily_change(inst_id):
    ...
    try:
        ...
        today, yesterday = daily.loc[0,'c'], daily.loc[1,'c']
...
def format_change_with_emoji(change):
    ...
    if change>=5: return f"🚨🚨🚨 (+{change:.2f}%)"
...
def get_24h_volume(inst_id):
    ...
    if df is None or len(df)<24: return 0
    return df['volCcyQuote'].sum()

# 🔹 메시지 전송
def send_top_volume_message(top_ids, volume_map):
    global sent_signal_coins, prev_positive_coins
    message_lines = ["⚡ 4H·1D MFI·RSI ≥70 필터","━━━━━━━━━━━━━━━━━━━"]
    ...
    for inst_id in top_ids:
        ...
        if not last_status and is_cross:
            ...
            if df_daily is None or len(df_daily)<3: 
                ...
                continue
            daily_mfi, daily_rsi = calc_mfi(df_daily,3).iloc[-1], calc_rsi(df_daily,3).iloc[-1]
            ...
            if inst_id in prev_positive_coins and daily_change:=calculate_daily_change(inst_id):
                if daily_change>0:
                    prev_positive_coins.add(inst_id)
                else:
    ...
    if current_signal_coins:
        ...
        message_lines+=["📌 BTC 현황",f"BTC {format_change_with_emoji(btc_change)} / 거래대금: ({btc_volume_str})","━━━━━━━━━━━━━━━━━━━"]
        ...
        for inst_id,daily_change,volume_24h,actual_rank in current_signal_coins:
            ...
            message_lines.append(f"{name} {format_change_with_emoji(daily_change)} / 거래대금: ({volume_str}) {actual_rank}위")
        message_lines.append("━━━━━━━━━━━━━━━━━━━")

        # 전체 조건 만족 TOP10
        all_coins=sorted(current_signal_coins,key=lambda x:x[2],reverse=True)[:10]
        message_lines.append("📊 전체 조건 만족 코인 TOP10")
        for rank,(inst_id,daily_change,volume_24h,actual_rank) in enumerate(all_coins,start=1):
            name = inst_id.replace("-USDT-SWAP","")
            volume_str=format_volume_in_eok(volume_24h) or "🚫"
            message_lines.append(f"{rank}. {name} {format_change_with_emoji(daily_change)} / 거래대금: ({volume_str}) {actual_rank}위")
        message_lines.append("━━━━━━━━━━━━━━━━━━━")
