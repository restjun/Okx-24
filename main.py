...
def ema_alignment_count(df):
    """
    업비트 4시간봉 확정 캔들만 전달받음.

    따라서:
    16:00~20:00 진행 중에는
    16:00 이전 마지막 확정봉 기준으로 카운팅.

    20:00 봉이 종료되면
    20:00 봉이 카운팅에 새로 포함됨.
    """
...
def is_buy(row):
    return bool(
        row and row.get("qualified")
    )

def is_short(row):
    return bool(
        row and row.get("short_qualified")
    )
...
def signal_html(row):
    ...
    if row.get(
        ...
        return (
            ...
            f'📉 진행 '
            f'{row["roc"].get("roc10_negative_count",0)}'
            '</b>'
        )
