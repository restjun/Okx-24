...
def get_enabled_all_filters():
    ...
    for timeframe in ["1H", "4H"]:
        ...
        for period in ROC_FILTER_PERIODS:
            ...
            if roc_settings()[period][timeframe] == "Y":
...
def roc_filter_html(
    ...
    for period in display_periods:
        ...
        if is_enabled:
            ...
            # 실제 필터
            parts.append(
                f'<span class="roc-item enabled">'
                f'<b>{period}</b>'
                f'{icon}'
                f'<small>{value_text}</small>'
                f'</span>'
            )
