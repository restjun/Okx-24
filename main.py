...
def send_top_volume_message(top_ids, volume_map):
    ...
    for inst_id in top_ids:
        # ✅ 두 조건 모두 만족해야 필터 통과
        if not (check_4h_mfi_rsi(inst_id, period=3, threshold=70) and check_daily_mfi_rsi(inst_id, period=3, threshold=70)):
            continue
        ...
        if is_positive: prev_positive_coins.add(inst_id)
        else: prev_positive_coins.discard(inst_id)
